...
impl repository::Company for Tsv {
...
mod tests {
    ...
    use crate::infrastructure::file_system::FileSystem;
    use crate::infrastructure::repositories::company::file_system::internal::tsv::Tsv;
    use anyhow::Result;
    ...
    async fn get_company_test() -> Result<()> {
        ...
        let actual = tsv.get_company(code, default_str).await?;
        let expected = model::Company {
            code: "1308".to_string(),
            name: "上場インデックスファンドＴＯＰＩＸ".to_string(),
            market: "T".to_string(),
            symbol: "1308.T".to_string(),
        };
